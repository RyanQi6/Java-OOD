...
public class BlackJackHand extends Hand{
    ...
    private int[] getScores(Card card){
        if(card.getFaceValuevalue() > 1){
            return new int[] {Math.min(card.getFaceValuevalue(),10)};
        }
        else {
            return new int[]{1, 11};
        }
    ...
    private static boolean isAce(Card c){
        return c.getFaceValuevalue() == 1;
    }
    private static boolean isFaceCard(Card c){
        return c.getFaceValuevalue() >= 11 && c.getFaceValuevalue() <=13;
    }
}
